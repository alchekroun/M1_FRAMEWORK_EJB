...
//	}
	...
	/*
	 * @Test public void testRetarderCorrespondance() throws NoSuchTrainException,
	 * CantCreateException, NoSuchArretException, CantDeleteException,
	 * NoSuchPassagerException {
	 * 
	 * Train train2 = factory.createTrainAvecResa();
	 * train2.setNom("Bordeaux - Paris - Marseille");
	 * train2.setDirectionType("forward"); train2.setStatut("enmarche");
	 * train2.setNumeroTrain(8621); train2.setReseau("SNCF");
	 * train2.setStatut("en marche");
	 * 
	 * Arret arret3 = factory.createArret(); arret3.setNom("Marseille");
	 * 
	 * List<Passager> listPassager = new ArrayList<Passager>(); for(int i=0; i<=49;
	 * i++) { Passager p = factory.createPassager(); p.setArrive(arret3);
	 * p.setDepart(arret1); listPassager.add(p); }
	 * 
	 * List<Integer> listIdPassager = new ArrayList<Integer>();; for(Passager p :
	 * listPassager) { listIdPassager.add(passagerService.createPassager(p)); }
	 * 
	 * int idTrain1 = trainService.createTrain(train1); int idTrain2 =
	 * trainService.createTrain(train2); int idArret1 =
	 * arretService.createArret(arret1); int idArret2 =
	 * arretService.createArret(arret2); int idArret3 =
	 * arretService.createArret(arret3);
	 * 
	 * arret1.setId(idArret1); arret2.setId(idArret2); arret3.setId(idArret3);
	 * train1.setId(idTrain1); train2.setId(idTrain2);
	 * 
	 * for(int i=0; i<listIdPassager.size(); i++) {
	 * listPassager.get(i).setId(listIdPassager.get(i)); }
	 * 
	 * em.getTransaction().begin(); trainService.monterListPassager(listPassager,
	 * dao.getTrainFromId(idTrain2)); em.getTransaction().commit();
	 * 
	 * LocalDateTime dt1 = LocalDateTime.now().minusMinutes(50); LocalDateTime dt2 =
	 * dt1.plusMinutes(10); String passage = dt1.toString() + " " + dt2.toString();
	 * String passage2 = dt1.plusMinutes(41).toString() + " " +
	 * dt2.plusMinutes(60).toString(); String passage5 =
	 * dt1.plusMinutes(200).toString() + " " + dt2.plusMinutes(240).toString();
	 * trainService.addArret(train1.getId(), arret1.getId(), passage, true, false);
	 * fr.pantheonsorbonne.ufr27.miage.jpa.HeureDePassage hdp1 =
	 * hdpDao.getHdpFromTrainIdAndArretId(train1.getId(), arret1.getId());
	 * trainService.addArret(train1.getId(), arret2.getId(), passage2, true, false);
	 * fr.pantheonsorbonne.ufr27.miage.jpa.HeureDePassage hdp2 =
	 * hdpDao.getHdpFromTrainIdAndArretId(train1.getId(), arret2.getId());
	 * trainService.addArret(train1.getId(), arret3.getId(), passage5, true, false);
	 * fr.pantheonsorbonne.ufr27.miage.jpa.HeureDePassage hdp5 =
	 * hdpDao.getHdpFromTrainIdAndArretId(train1.getId(), arret3.getId());
	 * 
	 * 
	 * LocalDateTime dt3 = dt1.plusMinutes(20); LocalDateTime dt4 =
	 * dt1.plusMinutes(30); String passage3 = dt3.toString() + " " + dt4.toString();
	 * String passage4 = dt3.plusMinutes(40).toString() + " " +
	 * dt4.plusMinutes(80).toString(); trainService.addArret(train2.getId(),
	 * arret1.getId(), passage3, true, false);
	 * fr.pantheonsorbonne.ufr27.miage.jpa.HeureDePassage hdp3 =
	 * hdpDao.getHdpFromTrainIdAndArretId(train2.getId(), arret1.getId());
	 * trainService.addArret(train2.getId(), arret2.getId(), passage4, true, false);
	 * fr.pantheonsorbonne.ufr27.miage.jpa.HeureDePassage hdp4 =
	 * hdpDao.getHdpFromTrainIdAndArretId(train2.getId(), arret2.getId());
	 * 
	 * for(Passager p : listPassager) { passagerDao.findTrajet(p.getId()); }
	 * 
	 ...
	 * Perturbation perturbation1 = new Perturbation();
	 * 
	 * perturbation1.setMotif("chevreuil"); perturbation1.setTrain(train2);
	 * perturbation1.setDureeEnPlus(100);
	 * trainService.createPerturbation(perturbation1);
	 * 
	 * trainService.retarderCorrespondance(train1);
	 * 
	 * fr.pantheonsorbonne.ufr27.miage.jpa.HeureDePassage hdpTrain1 =
	 * hdpDao.getHdpFromTrainIdAndArretId(train1.getId(), arret2.getId());
	 * 
	 * fr.pantheonsorbonne.ufr27.miage.jpa.HeureDePassage hdpTrain2 =
	 * hdpDao.getHdpFromTrainIdAndArretId(train2.getId(), arret2.getId());
	 * 
	 * assertTrue(hdpTrain1.getReelArriveeTemps().plusMinutes(10)!=hdpTrain1.
	 * getReelArriveeTemps());
	 * assertTrue(hdpTrain1.getReelDepartTemps().plusMinutes(10)!=hdpTrain1.
	 * getReelDepartTemps());
	 * 
	 * Passager p = factory.createPassager(); listPassager.add(p);
	 * p.setArrive(arret3); p.setDepart(arret1);
	 ...
	 * 
	 * trainService.retarderCorrespondance(train1);
	 * assertEquals(hdpTrain1.getReelArriveeTemps().plusMinutes(10),hdpTrain1.
	 * getReelArriveeTemps());
	 * assertEquals(hdpTrain1.getReelDepartTemps().plusMinutes(10),hdpTrain1.
	 * getReelDepartTemps());
	 * 
	 * for(int i=0; i<listIdPassager.size(); i++) {
	 * passagerService.deletePassager(listIdPassager.get(i)); }
	 * 
	 * arretService.deleteArret(idArret1); arretService.deleteArret(idArret2);
	 * arretService.deleteArret(idArret3); trainService.deleteTrain(idTrain1);

...
@NamedQueries({ @NamedQuery(name = "getAllPassager", query = "SELECT p FROM Passager p"),
		...
		@NamedQuery(name = "getPassagerByTrainIdAndNotArrivalAtArretId", query = "SELECT p FROM Passager p WHERE p.train.id= :trainId AND p.arrive.id<> :idArret AND p.correspondance.id= :idArret ")})
